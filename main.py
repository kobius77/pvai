...
def generate_ascii_table(df):
    ...
    for col in df_copy.columns:
        ...
        if df_copy[col].dtype == "object" or str(df_copy[col].dtype).startswith(
            "datetime"
        ):
    ...
    if tabulate:
        try:
            return "\n" + tabulate(
                df_copy.head(20), headers="keys", tablefmt="pipe", showindex=False
            )
...
async def lifespan(app: FastAPI):
    ...
    if OPENROUTER_API_KEY and OPENROUTER_API_KEY != "your_openrouter_key_here":
        try:
            ...
            print("Vanna initialized with OpenRouter + ChromaDB")
...
"""
        ...
        print("Vanna not configured - set OPENROUTER_API_KEY in .env")
        vn = None
...
if BUILD_DIR.exists():
    app.mount(
        "/static", StaticFiles(directory="src/client/build/static"), name="static"
    )
...
async def debug_data(limit: int = 5):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT timestamp, site_id, meter_id, export_energy, import_energy FROM energy_readings WHERE export_energy IS NOT NULL OR import_energy IS NOT NULL LIMIT %s",
            [limit],
        )
            ...
            result.append(
                {
                    ...
                    "site_id": r[1],
                    "meter_id": r[2],
                    "export_energy": float(r[3])
                    if r[3] and not math.isnan(r[3])
                    else None,
                    "import_energy": float(r[4])
                    if r[4] and not math.isnan(r[4])
                    else None,
                }
            )
        return result
...
async def get_data(year: str = "2024", month: str = "", day: str = ""):
    ...
    try:
        ...
        if day:
            ...
            """,
                [
                    f"{year}-{month.zfill(2)}-{day.zfill(2)}T00:00:00+00:00",
                    f"{year}-{month.zfill(2)}-{day.zfill(2)}T23:59:59+00:00",
                ],
            )
        elif month:
            ...
            """,
                [
                    ...
                    f"{int(year) + 1}-01-01T00:00:00+00:00",
                ],
            )
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    if "site_id" not in df.columns:
        ...
        if found_site:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"CSV must have site_id column. Found: {list(df.columns)}",
            )
    ...
    if "meter_id" not in df.columns:
        ...
        if found_meter:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"CSV must have meter_id column. Found: {list(df.columns)}",
            )
    ...
    if df["timestamp"].isna().any():
        print(
            f"WARNING: Dropping {df['timestamp'].isna().sum()} rows with invalid timestamps"
        )
    ...
    at_holidays = holidays.Austria(
        years=range(2020, 2030), subdiv=3
    )  # State 3 = Lower Austria
    ...
    df["workday"] = df.apply(
        lambda r: (
            r["date_only"] not in at_holidays
            and r["day_of_week"] not in ["Saturday", "Sunday"]
        ),
        axis=1,
    )
    ...
    try:
        ...
        if unique_sites:
            site_values = [
                [s[0], f"Site {s[0]}", "Auto-created from import"] for s in unique_sites
            ]
            execute_values(
                cursor,
                """
        ...
        if unique_meters:
            meter_values = [
                [m[0], m[1], f"Meter {m[0]}", "Auto-created from import"]
...
async def chat(request: dict):
    if vn is None:
        return {
            "error": "Vanna AI not configured. Set OPENROUTER_API_KEY in .env",
    ...
    try:
        ...
        if debug_mode:
            ...
            # Normal mode: Generate conversational summary with OpenRouter (temperature=0.7)
            ...
            if df is not None and not df.empty:
                ...
                # 3. Call LLM directly with Temperature = 0.7 for natural language
                try:
            ...
            if df is not None and not df.empty:
                ...
                if len(df) == 1:
                    ...
                    time_cols = [
                        col
                        for col in df.columns
                        if any(w in col.lower() for w in time_keywords)
                    ]
                    ...
                    if time_cols and other_cols:
                        try:
                            ...
                            dt_series = pd.to_datetime(
                                df[x_col], errors="coerce", utc=True
                            )
                            ...
                            if not dt_series.isna().all():
                                ...
                                elif (dt_series.dt.hour == 0).all() and (
                            ...
                            if len(df) > 25:
                                ...
                                fig.update_traces(
                                    marker=dict(size=5, color="#004973", opacity=0.8)
                                )
                            else:
        ...
        try:
            ...
            for table_name in tables:
                ...
                manifest["tables"].append(
                    {"name": table_name, "rows": len(df), "columns": list(df.columns)}
                )
    ...
    try:
        with zipfile.ZipFile(buffer, "r") as zf:
            if "manifest.json" not in zf.namelist():
                raise HTTPException(
                    status_code=400, detail="Backup file is missing manifest.json"
                )
            ...
            try:
                ...
                for name in zf.namelist():
                    if name.endswith(".csv"):
                        table_name = name[:-4]
                        print(
                            f"Database restore started: processing {table_name}...",
                            flush=True,
                        )
                            ...
                            if "timestamp" in df.columns:
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"Warning: Failed to calculate derived columns: {e}",
                                            flush=True,
                                        )
                                ...
                                if pk_cols:
                                    ...
                                    if set_clause:
                                        ...
                                        pk_placeholders = " AND ".join(
                                            [f"{pk} = EXCLUDED.{pk}" for pk in pk_cols]
                                        )
                                        upsert = f"""
                        ...
                        conn.commit()
                        print(
                            f"Successfully inserted {inserted_count} rows into {table_name}",
                            flush=True,
                        )

                        # Handle ChromaDB restore
                        chroma_files = [
                            n for n in zf.namelist() if n.startswith("chroma_data/")
                        ]
                        if manifest.get("includes_chromadb") and chroma_files:
                            print(
                                "Starting ChromaDB/Vanna vector store restoration...",
                                flush=True,
                            )
                            ...
                            for chroma_file in chroma_files:
                                ...
                                relative_path = chroma_file.replace(
                                    "chroma_data/", "", 1
                                )
                            ...
                            print(
                                "ChromaDB data restored. Re-instantiating client...",
                                flush=True,
                            )
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    print(
                                        "Vanna client re-instantiated successfully.",
                                        flush=True,
                                    )
                                except Exception as e:
                                    print(
                                        f"Warning: Failed to re-instantiate Vanna client: {e}",
                                        flush=True,
                                    )

                        elif (
                            manifest.get("includes_chromadb")
                            and "chroma.sqlite3" in zf.namelist()
                        ):
                            # Legacy single file restore
                            print(
                                "Starting ChromaDB/Vanna vector store restoration (legacy)...",
                                flush=True,
                            )
